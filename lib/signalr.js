...
var transportLogic = {
    ...
    send: function(connection, data) {
        ...
        function (err) {
            console.log("Error sending " + util.inspect(data, true, null) + " to " + url + " Error Details: " + err);
...
module.exports.SignalR = (function() {
    ...
    SignalR.prototype.start = function(options, callback) {
        ...
        if (typeof options === "function") {
            ...
            util.log("TODO: extend config with options");
        }
        var connection = this;
    ...
    SignalR.prototype.received = function(callback) {
        var connection = this;
        connection.on(SignalrEvents.onReceived, function (e, data) {
            callback.call(connection, data);
        });
        return connection;
    };

    var negotiate = function(signalR) {
        ...
        signalrHttp.post(
            ...
            null,
            function(body) {
                parseNegotiateBody(signalR, body);
            },
            function(e) {
    ...
    var parseNegotiateBody = function(signalR, body) {
        ...
        if (Array.isArray(signalR.transport)) {
            util.log("TODO: parseNegotiateBody");
            // todo
        } else {
            // default to "auto"
            transports = supportedTransports;
        }
        initialize(signalR, transports);
    };

    var initialize = function(signalR, transports, index) {
        index = index || 0;
        ...
        transport.start(signalR, function() {
            ...
            initialize(signalR, transports, index + 1);
        });
    };
        ...
        serverSentEvents: {
            ...
            start: function(connection, onSuccess, onFailed) {
                ...
                connection.eventSource.onopen = function(e) {
                    util.log("EventSource connected");
                ...
                connection.eventSource.onerror = function(e) {
                    ...
                    util.log("EventSource readyState: " + connection.eventSource.readyState);
                    if (e.eventPhase === EventSource.CLOSED) {
                        if (connection.eventSource.readyState === EventSource.CONNECTING) {
                            util.log("EventSource reconnecting due to the server connection ending");
                            that.reconnect(connection);
                        } else {
                            util.log("EventSource closed");
                            that.stop(connection);
                        }
                    } else {
                        util.log("EventSource error");
